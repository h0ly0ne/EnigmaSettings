...
namespace Krkadoni.EnigmaSettings
{
    ...
    public class Settings : ISettings
    {
        ...
        public IList<IService> FindServicesForSatellite(IXmlSatellite satellite)
        {
            return
                new List<IService>(
                    Services.Where(x => x.Transponder is ITransponderDVBS dvbs && dvbs.OrbitalPositionInt == Convert.ToInt32(satellite.Position)).ToList());
        }
            ...
            try
            {
                ...
                foreach (var position in missingPositions)
                {
                    ...
                    if (Satellites.Any(x => x.Position == mPosition.ToString(CultureInfo.CurrentCulture)))
                    {
                        ...
                        IList<ITransponderDVBS> found = FindTranspondersWithoutSatellite().Where(x => x.OrbitalPositionInt == Convert.ToInt32(xmlSat.Position)).ToList();
                        ...
                        Log.Warn(string.Format(Resources.Settings_AddMissingXMLSatellites_WARNING__Some_transponders_are_market_as_satellite_but_have_invalid_position_0_degrees_));
                }
                ...
                foreach (var newsat in newSatellites)
                {
                    ...
                    {
                        var mTran = tran;
                        IList<IXmlTransponder> query = newsat.Transponders.Where(t => ((Convert.ToInt32(t.Frequency) < Convert.ToInt32(mTran.Frequency) + frequencyToleration) && (Convert.ToInt32(t.Frequency) > Convert.ToInt32(mTran.Frequency) - frequencyToleration)) && t.Polarization == mTran.Polarization && t.SymbolRate == mTran.SymbolRate).ToList();
                        ...
                        Log.Warn(string.Format(Resources.Settings_AddMissingXMLSatellites_Added_new_transponder__0__for_previously_missing_satellite__1_, tran.TransponderId, newsat.PositionString));
                    }
                }
            }
            catch (Exception ex)
        ...
        public void MatchServicesWithTransponders()
        {
            ...
            try
            {
                var query = Services.Join(Transponders, sr => sr.TransponderId, tr => tr.TransponderId, (sr, tr) => new { Transponder = tr, Service = sr });

                foreach (var match in query.ToList())
                {
                    Log.Debug($"Service ID {match.Service.SID} {match.Service.Name} matched with transponder ID {match.Transponder.TransponderId}");
                    match.Service.Transponder = match.Transponder;
                }

                var servc = FindServicesWithoutTransponder();

                if (servc.Count == 0)
                    Log.Debug("All services have corresponding transponder");
                else
                {
                    foreach (var srv in servc)
                    {
                        Log.Warn(string.Format(Resources.Settings_MatchServicesWithTransponders_No_transponder_has_been_found_for_service__0_____1_, srv.ServiceId, srv.Name));
                    }
                }
            }
            catch (Exception ex)
            {
                Log.Error(ex.Message, ex);
                throw new SettingsException(Resources.Settings_MatchServicesWithTransponders_There_was_an_error_while_trying_to_match_services_with_transponders, ex);
            }
        }
        ...
        public void MatchSatellitesWithTransponders()
        {
            ...
            try
            {
                var transDVBS = FindSatelliteTransponders();
                ...
                foreach (var tr in transDVBS.Where(x => x.Satellite == null).ToList())
                {
                    Log.Warn(string.Format(Resources.Settings_MatchSatellitesWithTransponders_No_satellite_has_been_found_for_transponder__0_, tr.TransponderId));
                }
            }
            catch (Exception ex)
            {
                Log.Error(ex.Message, ex);
                throw new SettingsException(Resources.Settings_MatchSatellitesWithTransponders_There_was_an_error_while_matching_transponders_with_satellites_, ex);
            }
        }
        ...
        public void MatchFileBouquetsServices()
        {
            ...
            try
            {
                IList<IBouquetItemService> bis = Bouquets.SelectMany(x => x.BouquetItems).OfType<IBouquetItemService>().Concat(Bouquets.SelectMany(x => x.BouquetItems).OfType<IBouquetItemAlternative>().Where(a => a.Bouquet != null).SelectMany(a => a.Bouquet.BouquetItems).OfType<IBouquetItemService>()).ToList();
                ...
                {
                    foreach (var bqis in bq.BouquetItems.OfType<IBouquetItemService>().ToList().Where(bqis => bqis.Service != null))
                    {
        ...
        public void MatchBouquetsBouquetsServices()
        {
            ...
            try
            {
                var bouquetsBouquet = Bouquets.SingleOrDefault(x => x.BouquetType == Enums.BouquetType.E1Bouquets);
                ...
                foreach (var bq in bouquetsBouquet.BouquetItems.OfType<IBouquetItemBouquetsBouquet>().ToList())
                {
                    foreach (var bqis in bq.Bouquet.BouquetItems.OfType<IBouquetItemService>().ToList().Where(bqis => bqis.Service != null))
                    {
                        Log.Debug($"Bouquet item {bqis.ServiceId} in Enigma1 bouquet {bq.Bouquet.Name} matched to service {bqis.Service}");
                    }
                }
        ...
        public void RenumberBouquetFileNames()
        {
            try
            {
                ...
                {
                    var s = Path.GetFileName(x.FileName);
                    return s != null && (x.BouquetType == Enums.BouquetType.UserBouquetTv && s.StartsWith("userbouquet.dbe"));
                }).OrderBy(x => x.FileName).ToList();
                var index = 0;

                foreach (var bouquet in bqsTv)
                {
                    var name = Path.GetFileName(bouquet.FileName);

                    if (name != null && !name.StartsWith("userbouquet.dbe"))
                        continue;
                    ...
                    {
                        Log.Debug($"Changed filename for bouquet {bouquet.Name} from {Path.GetFileName(bouquet.FileName)} to {Path.GetFileName(fileName)}");
                ...
                foreach (var bouquet in bqsRadio)
                {
                    var fileName = string.Empty;
        ...
        public void RemoveEmptyMarkers(Func<IBouquetItemMarker, bool> preserveCondition)
        {
            try
            {
                ...
                foreach (var bouquet in Bouquets.Where(bouquet => bouquet.BouquetItems.Count > 0))
                {
                    for (var i = bouquet.BouquetItems.Count - 1; i >= 0; i += -1)
                    {
                        ...
                        if (bouquetItemMarker == null)
                            continue;

                        if (i == bouquet.BouquetItems.Count - 1)
                        {
                            if (preserveList is { Count: > 0 } && preserveList.Contains(bouquetItemMarker))
                                Log.Debug($"Marker {bouquetItemMarker.Description} in bouquet {bouquet.Name} is the last item, but not deleting it because it's matching preserve condition");
                            else
                            {
                                //if marker is last item in the list remove it
                                bouquet.BouquetItems.RemoveAt(i);
                                Log.Debug($"Empty marker {bouquetItemMarker.Description} removed from bouquet {bouquet.Name} because it's the last item in the bouquet");
                            }
                        }
                        else if (bouquet.BouquetItems.ElementAt(i + 1) is IBouquetItemMarker)
                        {
                            if (preserveList is { Count: > 0 } && preserveList.Contains(bouquetItemMarker))
                                Log.Debug($"Marker {bouquetItemMarker.Description} in bouquet {bouquet.Name} is suceeded by another marker, but not deleting it because it's matching preserve condition");
                            else
                            {
                                //if next item in the bouquet is also marker remove this one
                                bouquet.BouquetItems.RemoveAt(i);
                                Log.Debug($"Empty marker {bouquetItemMarker.Description} removed from bouquet {bouquet.Name} because it's suceeded by another marker");
                            }
                        }
                    }
                }
        ...
        private IList<IBouquet> AllServiceContainers()
        {
            ...
            {
                containers.Add(bq);
                containers.AddRange((from alternative in bq.BouquetItems.OfType<IBouquetItemAlternative>() where alternative.Bouquet != null select alternative.Bouquet));
            }
        ...
        public void RemoveStreams()
        {
            try
            {
                ...
                foreach (var bouquet in AllServiceContainers())
                {
        ...
        public void RemoveTransponder(ITransponder transponder)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new SettingsException(string.Format(Resources.Settings_RemoveTransponder_There_was_an_error_while_removing_transponder__0_, String.Empty), ex);
            }
        }
        ...
        public void RemoveSatellite(int orbitalPosition)
        {
            try
            {
                ...
                RemoveTransponders(trans);
                var satellite = Satellites.FirstOrDefault(x => x.Position == orbitalPosition.ToString(CultureInfo.CurrentCulture));
        ...
        public void RemoveInvalidBouquetItems()
        {
            try
            {
                ...
                foreach (var bouquet in AllServiceContainers())
                {
                    ...
                    IList<IBouquetItemFileBouquet> biFileBouquets = bouquet.BouquetItems.OfType<IBouquetItemFileBouquet>().Where(x => x.Bouquet == null).ToList();
                    ...
                    IList<IBouquetItemBouquetsBouquet> biBouquetsBouquets = bouquet.BouquetItems.OfType<IBouquetItemBouquetsBouquet>().Where(x => x.Bouquet == null).ToList();
                    ...
                    IList<IBouquetItemAlternative> biAlternatives = bouquet.BouquetItems.OfType<IBouquetItemAlternative>().Where(x => x.Bouquet == null).ToList();

                    foreach (var bia in biAlternatives)
                    {
                        bouquet.BouquetItems.Remove(bia);
                        Log.Debug($"Removed invalid reference to alternatives file {bia.FileName} from bouquet {bouquet}");
                    }
                }
