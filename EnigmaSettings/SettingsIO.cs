...
namespace Krkadoni.EnigmaSettings
{
    public class SettingsIO : ISettingsIO
    {
        ...
        public SettingsIO(IInstanceFactory factory, IFileProvider fileProvider, IPathProvider pathProvider)
        {
            if (factory == null)
        ...
        /// </summary>
        /// <remarks></remarks>
        public SettingsIO(IFileProvider fileProvider, IPathProvider pathProvider)
        {
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public ISettings Load(string settingsFile, IXmlSatellitesIO satellitesIO)
        {
            try
            {
                ...
                if (!_fileProvider.Exists(settingsFile))
                {
                ...
                try
                {
                    sats = satellitesIO.LoadSatellitesFromFile(_pathProvider.Combine(settings.SettingsDirectory, SatelliteXmlFileName)).ToList();
                }
                catch (Exception ex)
                {
                ...
                {
                    ReadFileBouquet(_pathProvider.Combine(settings.SettingsDirectory, UserBouquetTvEpl), ref settings);
                    ReadFileBouquet(_pathProvider.Combine(settings.SettingsDirectory, UserBouquetRadioEpl), ref settings);
                    ReadE1Bouquets(_pathProvider.Combine(settings.SettingsDirectory, BouquetsFile), ref settings);
                    settings.MatchBouquetsBouquetsServices();
                    settings.MatchFileBouquetsServices();
                    ReadServicesLocked(_pathProvider.Combine(settings.SettingsDirectory, ServicesLockedFile), ref settings);
                }
                ...
                {
                    ReadFileBouquet(_pathProvider.Combine(settings.SettingsDirectory, BouquetsTvFile), ref settings);
                    ReadFileBouquet(_pathProvider.Combine(settings.SettingsDirectory, BouquetsRadioFile), ref settings);
                    settings.MatchFileBouquetsServices();
                    ReadBlackWhiteList(_pathProvider.Combine(settings.SettingsDirectory, WhiteListFile), ref settings);
                    ReadBlackWhiteList(_pathProvider.Combine(settings.SettingsDirectory, BlackListFile), ref settings);
                }
        ...
        {
            IXmlSatellitesIO satellitesIO = _factory.InitNewXmlSatelliteIO(_fileProvider);
        ...
        /// <returns></returns>
        /// <remarks></remarks>
        public void LoadAsync(string settingsFile, IXmlSatellitesIO xmlSatellitesIO, AsyncCallback callback)
        {
            new Func<string, IXmlSatellitesIO, ISettings>(Load).BeginInvoke(settingsFile, xmlSatellitesIO, callback, null);
        }
        ...
        /// <remarks></remarks>
        public void Save(string folder, ISettings settings, IXmlSatellitesIO xmlSatellitesIO)
        {
            ...
            if (!_fileProvider.FolderExists(folder))
            {
                try
                {
                    _fileProvider.CreateFolder(folder);
            ...
            try
            {
                ...
                IXmlSatellitesIO satellitesIO = Factory.InitNewXmlSatelliteIO(_fileProvider);
                satellitesIO.SaveSatellitesToFile(_pathProvider.Combine(folder, SatelliteXmlFileName), settings);
                st.Stop();
        ...
        /// <param name="callback">Async callback to be called after save finishes</param>
        /// <remarks></remarks>
        public void SaveAsync(string folder, ISettings settings, IXmlSatellitesIO xmlSatellitesIO, AsyncCallback callback)
        {
            new Action<string, ISettings, IXmlSatellitesIO>(Save).BeginInvoke(folder, settings, xmlSatellitesIO, callback, null);
        }
        ...
        protected virtual string[] Read(string fileName, bool warnEmpty = false)
        {
            ...
            try
            {
                fileLines = _fileProvider.ReadAllLines(fileName);
            }
            catch (Exception ex)
            {
                Log.Error(ex.Message, ex);
        ...
        protected virtual string ReadText(string fileName, bool warnEmpty = true)
        {
            ...
            try
            {
                fileText = _fileProvider.ReadAllText(fileName);
            }
            catch (Exception ex)
            {
                Log.Error(ex.Message, ex);
        ...
        protected virtual void WriteFile(string fileName, string fContent, Encoding encoding)
        {
            ...
            try
            {
                _fileProvider.WriteAllText(fileName, fContent, encoding);
        ...
        protected virtual ISettings ReadSettingsFile(string fileName)
        {
            try
            {
                ISettings settings = _factory.InitNewSettings(_pathProvider);
        ...
        {
            Log.Debug(string.Format("Checking {0} to make sure we have valid settings", fileName));
            var fName = _pathProvider.GetFileName(fileName);
            ...
            if (!settingsFileLines[0].Trim().EndsWith("/"))
                throw new SettingsException(string.Format(Resources.Settings_ReadServicesFile_File__0__is_not_valid_settings_file_, fName));
            Log.Debug(string.Format("File {0} looks like a valid settings file, checking for version", fName));
        }
        ...
        protected virtual void ReadTransponders(ISettings settings, string[] settingsFileLines, string fileName, out int transponderOpenIndex,
            out int transponderEndIndex)
        {

            Log.Debug(string.Format("Reading transponders from {0}", fileName));
            ...
            if (transponderOpenIndex == -1)
                throw new SettingsException(string.Format(Resources.Settings_ReadServicesFile_No__transponder__tag_was_found_in__0_, _pathProvider.GetFileName(fileName)));
        ...
        {
            Log.Debug(string.Format("Reading services from {0}", fileName));
            ...
            if (servicesOpenIndex == -1)
                throw new SettingsException(string.Format(Resources.Settings_ReadServicesFile_No__services__tag_was_found_in_file__0_, _pathProvider.GetFileName(fileName)));
        ...
        protected virtual IFileBouquet ReadFileBouquet(string fileName, ref ISettings settings)
        {
            try
            {
                ...
                string fName = _pathProvider.GetFileName(fileName);
                bqt.FileName = fName;
                ...
                {
                    string dir = _pathProvider.GetDirectoryName(fileName);
            ...
            {
                Log.Error(ex.Message, ex);
                throw new SettingsException(string.Format(Resources.Settings_ReadBouquet_Failed_to_read_bouquet__0_, _pathProvider.GetFileName(fileName)), ex);
            }
        }
        ...
        protected virtual void DetectAndReadReference(string folder, ref IFileBouquet bouquet, string[] bouquetLines, int index, ISettings settings)
        {
            ...
            else if (line.StartsWith("#SERVICE"))
            {
                    ...
                    || favoritesType == Convert.ToInt32(Enums.FavoritesType.Unknown).ToString(CultureInfo.CurrentCulture))
                {
                    Log.Warn(string.Format(Resources.SettingsIO_ReadServicesLocked_Found_unknown_reference_type__0__in__1_, line,
                        _pathProvider.Combine(folder, bouquet.Name)));
                }
                         ...
                         || favoritesType == Convert.ToInt32(Enums.FavoritesType.File).ToString(CultureInfo.CurrentCulture))
                {
                    ...
                    string fName = _pathProvider.GetFileName(sData[10]);
                    if (fName == null) return;
                    IFileBouquet subBqt = ReadFileBouquet(_pathProvider.Combine(folder, fName), ref settings);
                    ...
                    if (subBqt != null)
                    {
                        ...
                        Log.Debug(string.Format("Bouquet {0} added as bouquet item to bouquet {1} ", _pathProvider.GetFileName(sData[10]), bouquet.Name));
                    }
                    else
                    {
                        Log.Warn(string.Format(Resources.Settings_ReadBouquet_Bouquet__0__in_bouquet__1__is_of_unknown_type, _pathProvider.GetFileName(sData[10]),
                            bouquet.Name));
                    }
                }
                else
            ...
            {
                Log.Warn(string.Format(Resources.Settings_ReadBouquet_Unsupported_line___1___in_bouquet__0_, _pathProvider.GetFileName(bouquet.FileName), line));
            }
        }
            ...
            try
            {
                ...
                for (int a = 0; a <= bouquetsE1.Count() - 2; a++)
                {
                    ...
                    if (bouquetLines.Length > 1)
                    {
                        ...
                        Log.Debug(string.Format("Found Enigma1 bouquet {0} in {1}", bouquetLines[1], fileName));
            ...
            try
            {
                ...
                foreach (string line in lockedLines)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Log.Debug(string.Format("Found reference to locked bouquet {0} in {1}", line, fileName));
                            try
                            {
                        ...
                        {
                            sLocked.Add(srv);
                            Log.Debug(string.Format("Found reference to locked service {0} in {1}", line, fileName));
                        }
                        else
                        {
        ...
        protected virtual void WriteSettingsFile(string directory, ISettings settings)
        {
            ...
            var fi = _pathProvider.Combine(directory, fileName);
            ...
            try
            {
                ...
                _fileProvider.WriteAllText(_pathProvider.Combine(directory, fileName), sContent.ToString());
                Log.Debug(string.Format("Sucessfully written {0} to {1}", fileName, _pathProvider.Combine(directory, fileName)));
            ...
            catch (Exception ex)
            {
                ...
                throw new SettingsException(
                    string.Format(Resources.SettingsIO_Save_Failed_to_save_settings_to__0_, _pathProvider.Combine(directory, fileName)), ex);
            }
        }
                    ...
                    case Enums.SettingsVersion.Enigma1:
                        if (tran.SystemType == Enums.DVBSSystemType.DVBS)
                        {
                            ...
                            sContent.Append("\t" + "s " + string.Join(":", new[]
                            {
                                ...
                                tran.Polarization,
                                tran.FEC,
                                tran.OrbitalPositionInt.ToString(CultureInfo.CurrentCulture),
                                tran.Inversion
                            }));
                            sContent.Append("\n" + "/" + "\n");
                        }
        ...
        protected virtual void WriteBouquet(string fileName, string content, string bouquetName)
        {
            ...
            {
                OnFileSaving(this, fileName);
                _fileProvider.WriteAllText(fileName, content);
            ...
            foreach (IBouquetItem bqItem in bouquet.BouquetItems)
            {
                switch (bqItem.BouquetItemType)
                {
                    ...
                    case Enums.BouquetItemType.FileBouquet:
                        ...
                        string fName = _pathProvider.GetFileName(bif.FileName);
                        if (fName == null)
                            throw new ArgumentNullException();
        ...
        protected virtual void WriteTvBouquetsE1(string directory, ISettings settings)
        {
                ...
                )
            {
                ...
                WriteBouquet(_pathProvider.Combine(directory, _pathProvider.GetFileName(bouquet.FileName)), bContent, bouquet.Name);
                bouquetIndex += 1;
            }
            WriteBouquet(_pathProvider.Combine(directory, UserBouquetTvEpl), sBouquetsTvContent.ToString(), UserBouquetTvEpl);
        }
        ...
        protected virtual void WriteTvBouquetsE2(string directory, ISettings settings)
        {
            ...
            {
                sBouquetsTvContent.Append("#SERVICE: 1:7:1:0:0:0:0:0:0:0:" + _pathProvider.GetFileName(bouquet.FileName) + "\n");
                Log.Debug(string.Format("Bouquet {0} added to bouquets.tv", _pathProvider.GetFileName(bouquet.FileName)));
                string bContent = E2BouquetToString(bouquet);
                string fName = _pathProvider.GetFileName(bouquet.FileName);
                if (fName != null) WriteBouquet(_pathProvider.Combine(directory, fName), bContent, bouquet.Name);
            }
            WriteBouquet(_pathProvider.Combine(directory, BouquetsTvFile), sBouquetsTvContent.ToString(), BouquetsTvFile);
        }
        ...
        protected virtual void WriteRadioBouquetsE1(string directory, ISettings settings)
        {
            ...
            WriteBouquet(_pathProvider.Combine(directory, UserBouquetRadioEpl), bouquetsRadioContent.ToString(), UserBouquetRadioEpl);
        }
        ...
        protected virtual void WriteRadioBouquetsE2(string directory, ISettings settings)
        {
            ...
            foreach (
                IFileBouquet bouquet in settings.Bouquets.OfType<IFileBouquet>().Where(x => x.BouquetType == Enums.BouquetType.UserBouquetRadio).ToList())
            {
                sBouquetsRadioContent.Append("#SERVICE: 1:7:2:0:0:0:0:0:0:0:" + _pathProvider.GetFileName(bouquet.FileName) + "\n");
                Log.Debug(string.Format("Bouquet {0} added to bouquets.radio", _pathProvider.GetFileName(bouquet.FileName)));
                string bContent = E2BouquetToString(bouquet);
                string fName = _pathProvider.GetFileName(bouquet.FileName);
                if (fName != null) WriteBouquet(_pathProvider.Combine(directory, fName), bContent, bouquet.Name);
            }
            WriteBouquet(_pathProvider.Combine(directory, BouquetsRadioFile), sBouquetsRadioContent.ToString(), BouquetsRadioFile);
        }
        ...
        protected virtual void WriteBlackList(string directory, ISettings settings)
        {
            ...
            WriteFile(_pathProvider.Combine(directory, BlackListFile), fContent.ToString(), Encoding.UTF8);
        }
        ...
        protected virtual void WriteWhiteList(string directory, ISettings settings)
        {
            ...
            WriteFile(_pathProvider.Combine(directory, WhiteListFile), fContent.ToString(), Encoding.UTF8);
        }
        ...
        protected virtual void WriteServicesLocked(string directory, ISettings settings)
        {
            try
            {
                ...
                string fName = _pathProvider.Combine(directory, ServicesLockedFile);
        ...
        protected virtual void WriteEnigma1Bouquets(string directory, ISettings settings)
        {
            try
            {
                ...
                string fName = _pathProvider.Combine(directory, BouquetsFile);
                DeleteFileIfExists(fName);
                    ...
                    if (existing.BouquetItems.Any())
                    {
                        ...
                        foreach (IBouquetItemBouquetsBouquet bbq in existing.BouquetItems.OfType<IBouquetItemBouquetsBouquet>().ToList())
                        {
                            if (bbq.Bouquet != null)
                            {
                                ...
                                {
                                    bbq.BouquetOrderNumberInt.ToString(CultureInfo.CurrentCulture),
                                    bbq.Bouquet.Name,
                                    ""
                                }));
                                    ...
                                    if (srv.Service != null)
                                    {
                                        if (srv.Service.Transponder != null)
                                        {
                                            fContent.Append(string.Join(":", new[]
                                            {
                                                ...
                                                Convert.ToInt32(srv.Service.ServiceType).ToString(CultureInfo.CurrentCulture)
                                            }) + "\n");
