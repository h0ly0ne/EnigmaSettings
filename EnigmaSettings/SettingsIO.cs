    ...
    public class SettingsIO : ISettingsIO
    {
        ...
        protected virtual void DetectAndReadReference(string folder, ref IFileBouquet bouquet, string[] bouquetLines, int index, ISettings settings)
        {
            ...
            else if (line.StartsWith("#SERVICE"))
            {
                         ...
                         || favoritesType == Convert.ToInt32(Enums.FavoritesType.File).ToString(CultureInfo.CurrentCulture))
                {
                    ...
                    if (tmpName.IndexOf(@"""", System.StringComparison.CurrentCultureIgnoreCase) > -1)
                    {
                        Log.Debug("Non standard bouquet name, try to workaround it");
                        tmpName = tmpName.Substring(tmpName.IndexOf(@"""", System.StringComparison.CurrentCultureIgnoreCase) + 1);
                        tmpName = tmpName.Substring(0, tmpName.IndexOf(@"""", System.StringComparison.CurrentCultureIgnoreCase));
        ...
        protected virtual void WriteSettingsFile(string directory, ISettings settings)
        {
            ...
            try
            {
                ...
                File.WriteAllText(Path.Combine(directory, fileName), sContent.ToString());
                Log.Debug(string.Format("Sucessfully written {0} to {1}", fileName, Path.Combine(directory, fileName)));
