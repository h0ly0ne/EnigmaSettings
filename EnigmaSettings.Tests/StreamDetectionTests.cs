...
namespace Krkadoni.EnigmaSettings.Tests
{
    public class StreamDetectionTests
    {
        ...
        [Fact]
        public void Stream_type_without_url_is_detected_by_type_set()
        {
            // Reference field has no "//" and the line has only 11 fields, so neither the URL
            // nor the 12-field heuristic applies - detection relies solely on the stream-type set.
            string dir = TestSettings.CopyFixtureToTemp("v4");
            File.WriteAllText(Path.Combine(dir, "userbouquet.stream.tv"),
                "#NAME Streams\n" +
                "#SERVICE 5001:0:1:1234:0:0:0:0:0:0:ChannelLabel\n" +
            ...
            var streamBq = settings.Bouquets.OfType<IFileBouquet>().Single(b => b.Name == "Streams");
            var stream = Assert.IsAssignableFrom<IBouquetItemStream>(streamBq.BouquetItems.Single());
            Assert.Equal("5001", stream.FavoritesTypeFlag);
        }
    }
}
